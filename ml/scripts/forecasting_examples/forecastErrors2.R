...
#========================
# ARMA models for NET_TOT.
NET_TOT_arima1 = arima(NET_TOT_ts_diff1, order = c(1, 0, 0), method = "ML")
NET_TOT_arima1

NET_TOT_arima2 = arima(NET_TOT_ts_diff1, order = c(0, 0, 1), method = "ML")
NET_TOT_arima2

NET_TOT_arima3 = arima(NET_TOT_ts_diff1, order = c(0, 0, 0), method = "ML")
NET_TOT_arima3

coeftest(NET_TOT_arima1)
coeftest(NET_TOT_arima2)
coeftest(NET_TOT_arima3)

#############################################################################################

# ARMA models for TOT_OP_EXP.
TOT_OP_EXP_arima1 = arima(TOT_OP_EXP_ts_diff1, order = c(1, 0, 0), method = "ML")
TOT_OP_EXP_arima1

TOT_OP_EXP_arima2 = arima(TOT_OP_EXP_ts_diff1, order = c(0, 0, 1), method = "ML")
TOT_OP_EXP_arima2

TOT_OP_EXP_arima3 = arima(TOT_OP_EXP_ts_diff1, order = c(0, 0, 0), method = "ML")
TOT_OP_EXP_arima3

coeftest(TOT_OP_EXP_arima1)
coeftest(TOT_OP_EXP_arima2)
coeftest(TOT_OP_EXP_arima3)

#############################################################################################

# ARMA models for NONOP_REV.
NONOP_REV_arima1 = arima(NONOP_REV_ts_diff1, order = c(1, 0, 0), method = "ML")
NONOP_REV_arima1

NONOP_REV_arima2 = arima(NONOP_REV_ts_diff1, order = c(0, 0, 1), method = "ML")
NONOP_REV_arima2

NONOP_REV_arima3 = arima(NONOP_REV_ts_diff1, order = c(0, 0, 0), method = "ML")
NONOP_REV_arima3

coeftest(NONOP_REV_arima1)
coeftest(NONOP_REV_arima2)
coeftest(NONOP_REV_arima3)

#========================
# Obtain BIC for models.
#========================
# BIC values for NET_TOT.
NET_TOT_arima1_bic = AIC(NET_TOT_arima1, k = log(length(NET_TOT_ts_diff1)))
NET_TOT_arima2_bic = AIC(NET_TOT_arima2, k = log(length(NET_TOT_ts_diff1)))
NET_TOT_arima3_bic = AIC(NET_TOT_arima3, k = log(length(NET_TOT_ts_diff1)))
...
# BIC values for TOT_OP_EXP.
TOT_OP_EXP_arima1_bic = AIC(TOT_OP_EXP_arima1, k = log(length(TOT_OP_EXP_ts_diff1)))
TOT_OP_EXP_arima2_bic = AIC(TOT_OP_EXP_arima2, k = log(length(TOT_OP_EXP_ts_diff1)))
TOT_OP_EXP_arima3_bic = AIC(TOT_OP_EXP_arima3, k = log(length(TOT_OP_EXP_ts_diff1)))

TOT_OP_EXP_arima1_bic
TOT_OP_EXP_arima2_bic
TOT_OP_EXP_arima3_bic

#############################################################################################

# BIC values for NONOP_REV.
NONOP_REV_arima1_bic = AIC(NONOP_REV_arima1, k = log(length(NONOP_REV_ts_diff1)))
NONOP_REV_arima2_bic = AIC(NONOP_REV_arima2, k = log(length(NONOP_REV_ts_diff1)))
NONOP_REV_arima3_bic = AIC(NONOP_REV_arima3, k = log(length(NONOP_REV_ts_diff1)))

NONOP_REV_arima1_bic
NONOP_REV_arima2_bic
NONOP_REV_arima3_bic

#########################################################
#====================Forecast Errors====================#
#########################################################
# Forecast for NET_TOT: I(1). Can be modeled with drift method.
NET_TOT_arima1_fore = forecast(NET_TOT_arima3, h = 10)
NET_TOT_arima1_fore

# Forecast for TOT_OP_EXP: I(1). Can be modeled with drift method.
TOT_OP_EXP_arima1_fore = forecast(TOT_OP_EXP_arima3, h = 10)
TOT_OP_EXP_arima1_fore

# Forecast for NONOP_REV: I(1). Can be modeled with drift method.
NONOP_REV_arima1_fore = forecast(NONOP_REV_arima3, h = 10)
NONOP_REV_arima1_fore

accuracy(NET_TOT_arima1_fore)
accuracy(TOT_OP_EXP_arima1_fore)
accuracy(NONOP_REV_arima1_fore)

layout(1:3)
plot(NET_TOT_arima1_fore)
plot(TOT_OP_EXP_arima1_fore)
plot(NONOP_REV_arima1_fore)
